...
export default function FinalCTA() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      if (prefersReducedMotion()) {
        gsap.set([readyRef.current, contentRef.current, scanRef.current], { clearProps: 'all' });
        return;
      }
  ...
  return (
    <section ref={sectionRef} id="contact" className="relative flex min-h-[92vh] items-center overflow-hidden py-32" style={{ paddingLeft: 'var(--page-pad-x)', paddingRight: 'var(--page-pad-x)', background: 'var(--ax-bg)' }}>
      <img src="/logoclear-320.webp" alt="" className="pointer-events-none absolute left-1/2 top-1/2 w-[92vw] max-w-[980px] -translate-x-1/2 -translate-y-1/2" style={{ opacity: 0.06, mixBlendMode: 'screen' }} />
      ...
      <div ref={scanRef} className="pointer-events-none absolute left-0 top-1/2 h-[2px] w-full" style={{ background: 'linear-gradient(90deg, transparent, var(--ax-lime), transparent)', boxShadow: '0 0 36px rgba(200,255,0,0.32)' }} />
